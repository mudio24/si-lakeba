...
ADD COLUMN `foto_bukti` VARCHAR(255) NOT NULL AFTER `ket`;

-- Query untuk menambahkan kolom estimasi_biaya dan bukti_invoice
-- Jalankan query ini di phpMyAdmin atau MySQL client
ALTER TABLE `pengaduan` 
ADD COLUMN `estimasi_biaya` DECIMAL(15,2) NULL DEFAULT NULL AFTER `foto_bukti`,
ADD COLUMN `bukti_invoice` VARCHAR(255) NULL DEFAULT NULL AFTER `estimasi_biaya`;
